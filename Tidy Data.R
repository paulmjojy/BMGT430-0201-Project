...
reduced_dataset <- kickstarter %>%
  ...
  select(goal, pledged, state, static_usd_rate, backers_count, launched_at_weekday, launched_at_month, launched_at_day, launched_at_yr, deadline_month, deadline_day, deadline_yr)
...
reduced_dataset$deadline_date <- as.Date(with(reduced_dataset, paste(deadline_month, deadline_day, deadline_yr, sep="/")), format="%m/%d/%Y")
reduced_dataset$launch_date <- as.Date(with(reduced_dataset, paste(launched_at_month, launched_at_day, launched_at_yr, sep="/")), format="%m/%d/%Y")
...
>>>>>>> 100418f1add9c6d9269d015c0a51a2b778867c1c
